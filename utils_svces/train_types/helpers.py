...
def create_attack_config(eps, steps, stepsize, norm, momentum=0.9, pgd='pgd', normalize_gradient=False, noise=None, use_fw=False):
    if noise is None:
...
def get_adversarial_attack(config, model, att_criterion, num_classes, epoch=0, args=None, Evaluator=None,
                           second_classifier=None, masks=1):
    ...
    if config['pgd'] == 'monotone':
        ...
        adv_attack = APGDAttack(model, eps=eps, n_iter=steps, norm=config['norm'], loss=att_criterion, use_fw=config['use_fw'], second_classifier=second_classifier, masks=masks)
