...
def MaxNets(dataset_name, arch, model_name_id, num_classes, img_size, device='cuda:0', project_folder=''):
    ...
    if '_ft' in model_name_id:
        ...
        ep = 3
        print(
            f'searching in {os.path.join(project_folder, temp_base, f"ft_ep_{ep}.pth")}')
        state_dict_file = \
            insensitive_glob(os.path.join(project_folder, temp_base, f"ft_ep_{ep}.pth"))[0]
    else:
        temp_base = 'FundusModels/robust'
        print(
            f'searching in {os.path.join(project_folder, temp_base, "0_25_l2.pth")}')
